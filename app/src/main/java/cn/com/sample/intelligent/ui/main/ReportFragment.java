    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.report_storage:
                reportType = WebSocketEnum.HuoQuKuFangLieBiao;
...
//        startDate.set(endDate.get(Calendar.YEAR), endDate.get(Calendar.MONTH), endDate.get(Calendar.DAY_OF_MONTH) - 7);
    ...
    WebSocketCallBack webSocketListener = new WebSocketCallBack() {
        ...
        public void onClosed(WebSocket webSocket, int code, String reason) {
            progressUtil.hideProgress(getActivity());
            if (reportType == WebSocketEnum.HuoQuRiBaoBiao) {
                dayReportList.clear();
    ...
    private void ReportSearch() {
        if (storageBean == null) {
            ...
            reportType = WebSocketEnum.HuoQuRiBaoBiao;
            WebSocketManage.getInstance().openConnect(webSocketListener, BaseConfig.HOST_WEBSOCKET);
        }
    }
    ...
    private void socket(WebSocket webSocket) {
        String data;
        if (reportType == WebSocketEnum.HuoQuKuFangLieBiao) {
            data = SharedPreferencesManager.getPhoneCode();
        } else if (reportType == WebSocketEnum.HuoQuRiBaoBiao) {
            data = storageBean.getID().concat("|").concat(reportStartTime.getText().toString())
                    .concat("|").concat(reportEndTime.getText().toString());
        } else {
            data = storageBean.getID();
        }

        WebSocketMsg msg1 = new WebSocketMsg();
        msg1.setHead("" + reportType);
        msg1.setType("0");
    ...
    private void socketData(String dataStr) {
        ...
        } else if (socketMsg.getHead().equals(WebSocketEnum.FanHuiRiBaoBiao + "")) {
            String output = AppUtil.toURLDecoder(socketMsg.getData());
            dayReportList.clear();
            try {
                JSONArray jsonArray = new JSONArray(output);
